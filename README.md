...
Vulnerability Dashboard is a complete UK Digital T Level classroom project: a readable Flask reference implementation plus a guided workbook designed for a broad range of learners. Students develop locally on their own laptop in VS Code, push to their own public GitHub repository, and deploy to a Linode cloud server reached only through its browser-based LISH console (SSH to the server is assumed blocked by the school network). Along the way they orient themselves on Linux, write core Python, build HTML/CSS/JavaScript pages from scratch, use Git and GitHub, understand JSON/APIs, normalise/cache vulnerability data, test failure paths, review security and deploy with Gunicorn, systemd and Nginx.
...
```text
Laptop (VS Code, git push) → GitHub (public repo, git pull) → Linode server (via LISH) →
Nginx → Gunicorn → Flask → file cache → NVD API / optional CISA KEV API
```
...
## Quick local setup
Run this on your own laptop (see STUDENT_GUIDE.md Lesson 4 for full VS Code/Python/Git install steps):
```bash
python3 -m venv .venv
...
python app.py
```
Open `http://127.0.0.1:5000`. The Flask development server is local-learning only; production uses Gunicorn behind Nginx on the Linode server, reached via LISH rather than SSH. See [deployment/DEPLOYMENT.md](deployment/DEPLOYMENT.md) for the full journey, including the public-repository `git clone`/`git pull` workflow that gets code onto the server with no SSH keys at all.
...
Security controls cover environment variables, non-root service operation, localhost Gunicorn, Nginx request limits, UFW plus Linode Cloud Firewall guidance (including closing port 22 to the public internet, since access is via LISH), validation, output encoding, timeout/error handling and security headers. Complete [SECURITY_REVIEW.md](SECURITY_REVIEW.md) and use [TROUBLESHOOTING.md](TROUBLESHOOTING.md). Run `pytest -q`, `ruff check .`, and `./scripts/check_setup.sh`.
